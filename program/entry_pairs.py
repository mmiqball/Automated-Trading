...
from private import any_open_positions
...
def open_positions(client):
  ...
  for index, row in df.iterrows():
    ...
    if len(series_1) > 0 and len(series_1) == len(series_2):
      ...
      z_score = zscore(spread).values.tolist()[-1]

      # Establish if potential trade
      if abs(z_score) >= ZSCORE_THRESHOLD:

        # Ensure like-for-like not already open (diversify trading)
        any_base_open = any_open_positions(client, base_market)
        any_quote_open = any_open_positions(client, quote_market)

        # Place trade
        if not any_base_open and not any_quote_open:
